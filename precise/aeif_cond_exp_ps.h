...
#ifndef AEIF_COND_EXP_PS_H
#define AEIF_COND_EXP_PS_H
...
/* BeginDocumentation
Name: ps_aeif_cond_exp - Conductance based exponential integrate-and-fire
...
to Brette and Gerstner (2005) and synaptic conductances are modelled as
exponential functions. This model implements precise spikes and a linear
interpolation to find spike times more precisely.
...
(exponential function).
  ...
  tau_syn_in double - Characteristic decrease time of the inhibitory synaptic
conductance in ms
(exponential function).
...
SeeAlso: iaf_cond_alpha, aeif_cond_exp, aeif_cond_alpha, aeif_cond_exp_ps
*/
 ...
 */
extern "C" int
aeif_cond_exp_ps_dynamics( double, const double*, double*, void* );

class aeif_cond_exp_ps : public Archiving_Node
{
public:
  /**
  ...
  */
  aeif_cond_exp_ps( const aeif_cond_exp_ps& );
...
private:
  ...
  friend class RecordablesMap< aeif_cond_exp_ps >;
  friend class UniversalDataLogger< aeif_cond_exp_ps >;
...
public:
  ...
  */
  struct Buffers_
  {
    Buffers_( aeif_cond_exp_ps& ); //!<Sets buffer pointers to 0
    Buffers_( const Buffers_&,
      aeif_cond_exp_ps& ); //!<Sets buffer pointers to 0

    //! Logger for all analog data
    UniversalDataLogger< aeif_cond_exp_ps > logger_;
  ...
  */
  ...
  static RecordablesMap< aeif_cond_exp_ps > recordablesMap_;

  /** @} */
};

inline port
aeif_cond_exp_ps::send_test_event( Node& target,
  port receptor_type,
  synindex,
  bool )
{
...
inline port
aeif_cond_exp_ps::handles_test_event( SpikeEvent&, port receptor_type )
{
  // You should usually not change the code in this function.
...
inline port
aeif_cond_exp_ps::handles_test_event( CurrentEvent&, port receptor_type )
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
inline port
aeif_cond_exp_ps::handles_test_event( DataLoggingRequest& dlr,
  port receptor_type )
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
inline void
aeif_cond_exp_ps::get_status( DictionaryDatum& d ) const
{
...
inline void
aeif_cond_exp_ps::set_status( const DictionaryDatum& d )
{
...
#endif /* #ifndef AEIF_COND_EXP_PS_H */
