...
#ifndef AEIF_PSC_ALPHA_FAST_H
#define AEIF_PSC_ALPHA_FAST_H
...
public:
  aeif_psc_alpha_fast();
  aeif_psc_alpha_fast( const aeif_psc_alpha_fast& );
  ~aeif_psc_alpha_fast();
...
private:
  ...
  friend int aeif_psc_alpha_fast_dynamics( double, const double*, double*, void* );

  // The next two classes need to be friends to access the State_ class/member
  friend class RecordablesMap< aeif_psc_alpha_fast >;
  friend class UniversalDataLogger< aeif_psc_alpha_fast >;
...
public:
  ...
  struct Buffers_
  {
    Buffers_( aeif_psc_alpha_fast& );                  //!<Sets buffer pointers to 0
    Buffers_( const Buffers_&, aeif_psc_alpha_fast& ); //!<Sets buffer pointers to 0

    //! Logger for all analog data
    UniversalDataLogger< aeif_psc_alpha_fast > logger_;
  ...
  static RecordablesMap< aeif_psc_alpha_fast > recordablesMap_;
};

inline port
aeif_psc_alpha_fast::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
aeif_psc_alpha_fast::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
aeif_psc_alpha_fast::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
aeif_psc_alpha_fast::handles_test_event( DataLoggingRequest& dlr,
  rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline void
aeif_psc_alpha_fast::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
aeif_psc_alpha_fast::set_status( const DictionaryDatum& d )
{
