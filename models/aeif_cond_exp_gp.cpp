...
nest::RecordablesMap< nest::aeif_cond_exp_gp >
  nest::aeif_cond_exp_gp::recordablesMap_;
...
template <>
void
RecordablesMap< aeif_cond_exp_gp >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m,
    &aeif_cond_exp_gp::get_y_elem_< aeif_cond_exp_gp::State_::V_M > );
  insert_( names::g_ex,
    &aeif_cond_exp_gp::get_y_elem_< aeif_cond_exp_gp::State_::G_EXC > );
  insert_( names::g_in,
    &aeif_cond_exp_gp::get_y_elem_< aeif_cond_exp_gp::State_::G_INH > );
  insert_(
    names::w, &aeif_cond_exp_gp::get_y_elem_< aeif_cond_exp_gp::State_::W > );
}
}

extern "C" int
nest::aeif_cond_exp_gp_dynamics( double,
  const double y[],
  double f[],
  void* pnode )
{
  // a shorthand
  typedef nest::aeif_cond_exp_gp::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::aeif_cond_exp_gp& node =
    *( reinterpret_cast< nest::aeif_cond_exp_gp* >( pnode ) );
...
nest::aeif_cond_exp_gp::Parameters_::Parameters_()
...
nest::aeif_cond_exp_gp::State_::State_( const Parameters_& p )
  : r_( 0 )
  , r_offset_( 0. )
{
  y_[ 0 ] = p.E_L;
...
nest::aeif_cond_exp_gp::State_::State_( const State_& s )
  : r_( s.r_ )
...
void
nest::aeif_cond_exp_gp::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_cond_exp_gp::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::aeif_cond_exp_gp::State_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_cond_exp_gp::State_::set( const DictionaryDatum& d,
  const Parameters_& )
{
...
nest::aeif_cond_exp_gp::Buffers_::Buffers_( aeif_cond_exp_gp& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_cond_exp_gp::Buffers_::Buffers_( const Buffers_&,
  aeif_cond_exp_gp& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
...
nest::aeif_cond_exp_gp::aeif_cond_exp_gp()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
nest::aeif_cond_exp_gp::aeif_cond_exp_gp( const aeif_cond_exp_gp& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::aeif_cond_exp_gp::init_state_( const Node& proto )
{
  const aeif_cond_exp_gp& pr = downcast< aeif_cond_exp_gp >( proto );
  S_ = pr.S_;
}

void
nest::aeif_cond_exp_gp::init_buffers_()
{
  ...
  B_.sys_.function = aeif_cond_exp_gp_dynamics;
...
void
nest::aeif_cond_exp_gp::calibrate()
{
  B_.logger_
...
void
nest::aeif_cond_exp_gp::interpolate_( double& t, double t_old )
{
...
void
nest::aeif_cond_exp_gp::handle( SpikeEvent& e )
{
...
void
nest::aeif_cond_exp_gp::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::aeif_cond_exp_gp::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
