...
nest::RecordablesMap< nest::aeif_psc_alpha_fast >
  nest::aeif_psc_alpha_fast::recordablesMap_;
...
template <>
void
RecordablesMap< aeif_psc_alpha_fast >::create()
{
  // use standard names whereever you can for consistency!
  insert_(
    names::V_m, &aeif_psc_alpha_fast::get_y_elem_< aeif_psc_alpha_fast::State_::V_M > );
  insert_( names::I_syn_ex,
    &aeif_psc_alpha_fast::get_y_elem_< aeif_psc_alpha_fast::State_::I_EXC > );
  insert_( names::I_syn_in,
    &aeif_psc_alpha_fast::get_y_elem_< aeif_psc_alpha_fast::State_::I_INH > );
  insert_(
    names::w, &aeif_psc_alpha_fast::get_y_elem_< aeif_psc_alpha_fast::State_::W > );
}
}

extern "C" int
nest::aeif_psc_alpha_fast_dynamics( double,
  const double y[],
  double f[],
  void* pnode )
{
  // a shorthand
  typedef nest::aeif_psc_alpha_fast::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::aeif_psc_alpha_fast& node =
    *( reinterpret_cast< nest::aeif_psc_alpha_fast* >( pnode ) );
...
nest::aeif_psc_alpha_fast::Parameters_::Parameters_()
  : V_peak_( 0.0 )    // mV
  , V_reset_( -60.0 ) // mV
  , t_ref_( 0.0 )     // ms
  , g_L( 30.0 )       // nS
...
nest::aeif_psc_alpha_fast::State_::State_( const State_& s )
  : r_( s.r_ )
{
...
nest::aeif_psc_alpha_fast::State_& nest::aeif_psc_alpha_fast::State_::operator=(
  const State_& s )
{
...
void
nest::aeif_psc_alpha_fast::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_psc_alpha_fast::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::aeif_psc_alpha_fast::State_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_psc_alpha_fast::State_::set( const DictionaryDatum& d,
  const Parameters_& )
{
...
nest::aeif_psc_alpha_fast::Buffers_::Buffers_( aeif_psc_alpha_fast& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_psc_alpha_fast::Buffers_::Buffers_( const Buffers_&, aeif_psc_alpha_fast& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_psc_alpha_fast::aeif_psc_alpha_fast()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
nest::aeif_psc_alpha_fast::aeif_psc_alpha_fast( const aeif_psc_alpha_fast& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::aeif_psc_alpha_fast::init_state_( const Node& proto )
{
  const aeif_psc_alpha_fast& pr = downcast< aeif_psc_alpha_fast >( proto );
  S_ = pr.S_;
}

void
nest::aeif_psc_alpha_fast::init_buffers_()
{
  ...
  B_.sys_.function = aeif_psc_alpha_fast_dynamics;

  B_.I_stim_ = 0.0;
}

void
nest::aeif_psc_alpha_fast::calibrate()
{
...
void
nest::aeif_psc_alpha_fast::update( Time const& origin,
  const long from,
  const long to )
{
  assert(
...
void
nest::aeif_psc_alpha_fast::handle( SpikeEvent& e )
{
...
void
nest::aeif_psc_alpha_fast::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::aeif_psc_alpha_fast::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
